...
public class DataMapLoader implements PreparableReloadListener, IdentifiableResourceReloadListener {
    ...
    private <T> void apply(MappedRegistry<T> registry, LoadResult<T> result) {
        ...
        result.results().forEach((key, entries) -> ((IRegistryWithData<T>) registry).getDataMaps().put(
                key, this.buildDataMap(registry, key, (List) entries)));
        DataMapsUpdatedCallback.EVENT.invoker().onDataMapsUpdated(registryAccess, registry, DataMapsUpdatedCallback.UpdateCause.SERVER_RELOAD);
    }
